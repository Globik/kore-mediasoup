...
//printf("***********************************************8ON_DATA occured: %s\n",(char*)data);
	printf("************************on_data********************************\n");
	//fucker((const char*)"fireeeeeee888&****");
return NULL;
}
...
//fucker(f);
//	int rc=uv_callback_fire(&send_data,(void*)"TO_SENNNNNNDDD_DATA",NULL);
//	if(rc==0){printf("cb_fire is OK.\n");}else{printf("cb_fire is NOT OK\n");}
}
...
// cd kore-mediasoup/addon/emitter2
...
if(!c){printf("!c\n");return;}
	...
	//int rc=uv_callback_fire(&send_data,NULL,NULL);
	//if(rc==0){printf("cb_fire is OK.\n");}else{printf("cb_fire is NOT OK\n");}
	uv_loop_t *loop=uv_default_loop();//NULL;
	uv_loop_init(loop);
	/*
	napi_status sta=napi_get_uv_event_loop(env,&loop);
	if(sta==napi_ok){
		printf("loop is ok\n");
	}else{
	...
	*/
	...
	int rc=uv_callback_init(loop,&send_data,on_data, UV_DEFAULT);
	if(rc==0){printf("cb_init OK.\n");}else{printf("cb_init NOT OK.\n");}
	//int
	rc=uv_callback_fire(&send_data,(void*)"SUUUUUUUUUUUUUKA",NULL);
	if(rc==0){printf("cb_fire is OK.\n");}else{printf("cb_fire is NOT OK\n");}
	//sleep(1);
	
	printf("NACHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH\n");
	//return;
...
size_t argc=3;
	...
	*/
	...
	//int rc=uv_callback_init(loop,&send_data,on_data, UV_DEFAULT);
	//if(rc==0){printf("cb_init OK.\n");}else{printf("cb_init NOT OK.\n");}
	// rc=uv_callback_fire(&send_data,NULL,NULL);
	//if(rc==0){printf("cb_fire is OK.\n");}else{printf("cb_fire is NOT OK\n");}
	//uv_run(loop,UV_RUN_DEFAULT);
...
//printf("status async: %d\n",ab);
	...
	// cd kore-mediasoup/addon/emitter2 
	printf("before delete ref shenv dcb\n");
	if(shenv==NULL){printf("shenv is NULL.\n");}else{
		printf("shenv is NOT NULL.\n");
	}
	uv_loop_t *loop=NULL;
	
	napi_status sta=napi_get_uv_event_loop(menv,&loop);
	if(sta==napi_ok){
		printf("loop is ok\n");
	}else{
		printf("loop is NOT ok\n");
...
if(k==napi_ok){ printf("del_ref2 is ok\n");}else{printf("del_ref2 is not ok\n");}
//usleep(10000000);
