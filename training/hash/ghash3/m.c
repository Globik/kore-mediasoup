...
typedef void(*on_fuck)();
typedef void(*on_duck)();
struct room
{
int b;
on_close close;
...
struct room*init_room(on_close cb){
struct room*r=(struct room*)calloc(1,sizeof*r);
if(r==NULL)return NULL;
r->b=204;
r->close=cb;//this.on('close')
//r->onfuck();
return r;
}

void dcb(){printf("on_close occured.\n");}
void bcb(){printf("bcb occured.\n");}
void bob(){printf("Bob occured.\n");}

struct worker*init_worker(on_fuck fk,on_duck dc){
struct worker*w=(struct worker*)calloc(1,sizeof*w);
if(w==NULL)return NULL;
w->a=2;
w->onduck=dc;
w->vroom=init_room(dcb);
if(w->vroom==NULL){free(w);return NULL;}
//w->vroom->close();//this.emit("close")
w->vroom->onfuck=fk;
...
/*
void*on_fire_answer(void*data){
if(data=="room_created")

}
*/
void create_room(struct worker*w){
// fire("create.room",on_fire_answer)
	//w
}
int main(){
struct worker*w=init_worker(bcb,bob);
if(w==NULL)exit(1);
int s=w->vroom->b;
printf("zifra: %d\n",s);
w->vroom->close();//this.emit('close')
w->vroom->onfuck();
w->onduck();//this.emit('duck')?
free(w->vroom);
free(w);
return 0;
}
